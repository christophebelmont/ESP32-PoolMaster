...
/* Return true if the page had just changed and false on the next loop */
bool EasyNex::hasPageChanged()
{
  if(pageChangedAction==false) {
    pageChangedAction = true;
    return true;
  }
  return false;
}
...
bool EasyNex::NextionListen(){
  ...
  {
    this->writeStr(F("thup=1"));
    this->writeStr(F("wup=1"));     // Wake up on last page
    this->writeStr(F("usup=1"));    // Authorize auto wake up on serial data
    this->writeStr(F("sleep=1"));
    Nextion_Sleeping = true;
  }

  return (!Nextion_Sleeping);
}

void EasyNex::Deactivate_Sleep(){
  Nextion_CanSleep = false;
}

void EasyNex::Activate_Sleep() {
